...
const verdictSchema = z.object({
    ...
    summary_text: z.string().max(500, "Max 500 characters").min(10, "Min 10 characters"),
...
export function VerdictForm({ companyId, initialData, onSaved, onDirtyChange }: VerdictFormProps) {
    ...
    const defaultValues: VerdictFormValues = {
        ...
        linked_report_ids: initialData?.linked_reports?.map(r => r.report_id) || [],
    };

    const [lockVersion, setLockVersion] = useState(initialData?.lock_version || 0);
    const [linkedReports, setLinkedReports] = useState<{ id: string, name: string }[]>(
        initialData?.linked_reports?.map(r => ({ id: r.report_id, name: r.filename })) || []
    );
    ...
    const removeStrength = (idx: number) => setValue("strengths", strengths.filter((_, i) => i !== idx));
    ...
    const removeWeakness = (idx: number) => setValue("weaknesses", weaknesses.filter((_, i) => i !== idx));
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setLinkedReports(prev => [...prev, { id: doc.id, name: doc.title || file.name }]);
            toast({ title: "Report uploaded" });
        } catch (err: any) {
            console.error(err);
            toast({ title: "Upload failed", variant: "destructive", description: err.message || "Could not upload document" });
        } finally {
    ...
    const onSubmit = async (data: VerdictFormValues) => {
        ...
        try {
            const updateReq: VerdictUpdateRequest = {
                ...
                final_verdict: data.final_verdict as any // Cast enum
            };
                ...
                if (details && details.current_state) {
                    setConflictStart({
                        your: {
                            ...
                            final_verdict: data.final_verdict as any
                        },
                        server: details.current_state as VerdictResponse
                    });
                }
            } else {
                toast({ title: "Error saving verdict", description: error.message || "Unknown error", variant: "destructive" });
            }
        } finally {
    ...
    const handleConflictResolve = (resolution: VerdictUpdateRequest) => {
        reset({
            final_verdict: (resolution.final_verdict as any) || undefined,
        ...
        toast({ title: "Conflict resolved", description: "Please review and save again." });
    };
                                ...
                                <div className="flex flex-col sm:flex-row gap-4">
                                    {["INVEST", "WATCHLIST", "PASS", "NO_THESIS"].map(opt => (
                                        <label key={opt} className="flex items-center space-x-2 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-sm font-medium">{opt.replace('_', ' ')}</span>
                                        </label>
                                    ))}
                                </div>
                            )}
                        />
                        {errors.final_verdict && <p className="text-sm font-medium text-destructive">{errors.final_verdict.message}</p>}
                    </div>

                    {/* Summary Text */}
                    <div className="space-y-3">
                        <Label htmlFor="summary_text">Investment Summary (50-100 words)</Label>
                        <Textarea
                            id="summary_text"
                        ...
                        />
                        {errors.summary_text && <p className="text-sm font-medium text-destructive">{errors.summary_text.message}</p>}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={addStrength} className="w-full">
                                    <Plus className="h-3 w-3 mr-2" /> Add Strength
                                </Button>
                            </div>
                            {errors.strengths && (
                                <p className="text-sm font-medium text-destructive">{errors.strengths.message}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={addWeakness} className="w-full">
                                    <Plus className="h-3 w-3 mr-2" /> Add Weakness
                                </Button>
                            </div>
                            {errors.weaknesses && (
                                <p className="text-sm font-medium text-destructive">{errors.weaknesses.message}</p>
                            )}
                        </div>
                    </div>

                    <div className="space-y-3">
                        <Label htmlFor="guidance_summary">Management Guidance (Optional)</Label>
                        <Textarea
                            id="guidance_summary"
                        ...
                        />
                        {errors.guidance_summary && <p className="text-sm font-medium text-destructive">{errors.guidance_summary.message}</p>}
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            {linkedReports.map(report => (
                                <div key={report.id} className="flex items-center p-2 border rounded bg-slate-50 dark:bg-slate-900 text-sm">
                            ...
                            <div className="relative border border-dashed rounded flex flex-col items-center justify-center p-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors cursor-pointer group">
                                {isUploading ? (
                                    ...
                                    <>
                                        ...
                                        <span className="text-xs text-muted-foreground">Attach Report</span>
                                    </>
                                )}
                                <input
                                    type="file"
