...
const DEFAULT_DOC_TYPES: DocTypeConfig[] = [
  ...
  { id: "10k", label: "Annual Report (10-K)", apiType: "10-k" },
  {
    ...
    apiType: "presentation",
  },
  {
    id: "analyst",
    label: "Analyst Report",
    apiType: "analyst_report",
    uploadable: true,
  },
];
      ...
      {periods.map((period) => {
        ...
        const matchingDoc = documents.find((d) => {
          ...
          if (isQuarterly) {
            ...
            return (
              d.fiscal_year === year &&
              d.fiscal_quarter === quarter &&
              d.document_type === docType.apiType
            );
          } else {
            ...
            return (
              d.fiscal_year === year && d.document_type === docType.apiType
            );
          }
        });
  ...
  const [docOrder, setDocOrder] = useState(() => {
    ...
    if (savedOrder) {
      try {
        ...
        const missing = DEFAULT_DOC_TYPES.filter(
          (d) => !orderIds.includes(d.id),
        );
    ...
    if (over && active.id !== over.id) {
      setDocOrder((items) => {
        ...
        localStorage.setItem(
          "document_grid_order",
          JSON.stringify(newOrder.map((i: DocTypeConfig) => i.id)),
        );
        return newOrder;
      });
    }
