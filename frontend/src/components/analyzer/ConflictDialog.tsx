    ...
    const handleMerge = () => {
        ...
        const merged: VerdictUpdateRequest = {
            ...
            summary_text: [
                ...
                serverVersion.summary_text !== yourVersion.summary_text ? `\n[Server Version]:\n${serverVersion.summary_text}` : null
            ].filter(Boolean).join('\n\n'),
            strengths: Array.from(new Set([...yourVersion.strengths, ...serverVersion.strengths])),
            weaknesses: Array.from(new Set([...yourVersion.weaknesses, ...serverVersion.weaknesses])),
            guidance_summary: [
                yourVersion.guidance_summary,
                serverVersion.guidance_summary !== yourVersion.guidance_summary ? `\n[Server Version]:\n${serverVersion.guidance_summary}` : null
            ].filter(Boolean).join('\n\n'),
            linked_report_ids: Array.from(new Set([...yourVersion.linked_report_ids, ...serverVersion.linked_reports.map(r => r.report_id)])),
        };
        onResolve(merged);
        onOpenChange(false);
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Another user has updated this verdict since you loaded it.
                        Please choose how you want to resolve this conflict.
                    </DialogDescription>
                </DialogHeader>

                <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-4 border rounded-md p-4 min-h-0">
                    <div className="space-y-4">
                        <h3 className="font-semibold text-lg border-b pb-2 mb-2">Your Version</h3>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="text-sm bg-muted/30 p-2 rounded whitespace-pre-wrap">
                                {yourVersion.summary_text || <span className="italic text-muted-foreground">Empty</span>}
                            </div>
                        </div>

                        <div className="space-y-1">
                            <span className="text-xs font-medium text-muted-foreground uppercase">Strengths</span>
                            <ul className="list-disc list-inside text-sm">
                                {yourVersion.strengths.length ? yourVersion.strengths.map((s, i) => (
                                    <li key={i}>{s}</li>
                                )) : <li className="list-none text-muted-foreground italic">None</li>}
                            </ul>
                        </div>
                    </div>

                    <div className="space-y-4 bg-slate-50 dark:bg-slate-900/50 -m-4 p-4 border-l">
                        <h3 className="font-semibold text-lg border-b pb-2 mb-2">Server Version</h3>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="text-sm bg-background p-2 rounded border whitespace-pre-wrap">
                                {serverVersion.summary_text || <span className="italic text-muted-foreground">Empty</span>}
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            <span className="text-xs font-medium text-muted-foreground uppercase">Strengths</span>
                            <ul className="list-disc list-inside text-sm">
                                {serverVersion.strengths.length ? serverVersion.strengths.map((s, i) => (
                                    <li key={i}>{s}</li>
                                )) : <li className="list-none text-muted-foreground italic">None</li>}
                            </ul>
                        </div>
