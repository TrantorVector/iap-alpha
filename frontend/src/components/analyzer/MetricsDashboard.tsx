                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {metrics.map((metric) => {
                                ...
                                const invertColors = metric.metric_name.includes('_ratio') ||
...
export function MetricsDashboard({ data, isLoading }: MetricsDashboardProps) {
            ...
            <div className="space-y-4 p-6">
                <div className="space-y-4">
                    {Object.keys(SECTION_TITLES).map((sectionKey) => (
                        <div key={sectionKey} className="border border-gray-200 rounded-lg p-4 bg-white">
    ...
    return (
        <div className="space-y-4 p-6 bg-gray-50">
            <div className="mb-4">
                <h2 className="text-2xl font-bold text-gray-900">Key Metrics Dashboard</h2>
                <p className="text-sm text-gray-600 mt-1">
                    {data.period_type === 'quarterly' ? 'Quarterly' : 'Annual'} metrics for {data.periods.length} periods
                </p>
            </div>
