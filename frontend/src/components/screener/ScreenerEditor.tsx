...
const schema = z.object({
    ...
    filter_criteria: z.object({
        ...
        market_cap_min: z.coerce.number().optional().or(z.literal("")).transform(v => v === "" ? undefined : v),
        market_cap_max: z.coerce.number().optional().or(z.literal("")).transform(v => v === "" ? undefined : v),
        momentum_1m_min: z.coerce.number().optional().or(z.literal("")).transform(v => v === "" ? undefined : v),
        momentum_3m_min: z.coerce.number().optional().or(z.literal("")).transform(v => v === "" ? undefined : v),
        momentum_6m_min: z.coerce.number().optional().or(z.literal("")).transform(v => v === "" ? undefined : v),
...
export function ScreenerEditor({ mode, initialData, onSave, onClose, open }: ScreenerEditorProps) {
    const {
        register,
        handleSubmit,
        reset,
    ...
    const toggleSelection = (field: 'exchanges' | 'sectors' | 'verdict_types', value: string) => {
        const current = watch(`filter_criteria.${field}`) || [];
        if (current.includes(value)) {
            setValue(`filter_criteria.${field}`, current.filter((v: string) => v !== value));
        } else {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{mode === 'create' ? "Create New Screener" : "Edit Screener"}</DialogTitle>
                    <DialogDescription>
                        Define your filtering criteria to find the best investment opportunities.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="space-y-6 border-t pt-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {EXCHANGES.map(exchange => (
                                    <Badge
                                        key={exchange}
                                        variant={watch("filter_criteria.exchanges")?.includes(exchange) ? "default" : "outline"}
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {SECTORS.map(sector => (
                                    <Badge
                                        key={sector}
                                        variant={watch("filter_criteria.sectors")?.includes(sector) ? "default" : "outline"}
                                        className="cursor-pointer px-3 py-1"
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <Label htmlFor="market_cap_min" className="text-[10px] text-muted-foreground">Min</Label>
                                    <Input
                                        id="market_cap_min"
                                        type="number"
                                        placeholder="Min"
                                ...
                                <div className="space-y-1">
                                    <Label htmlFor="market_cap_max" className="text-[10px] text-muted-foreground">Max</Label>
                                    <Input
                                        id="market_cap_max"
                                        type="number"
                                        placeholder="Max"
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div className="space-y-1">
                                    <Label htmlFor="momentum_1m_min" className="text-[10px] text-muted-foreground">1 Month</Label>
                                    <Input
                                        id="momentum_1m_min"
                                        type="number"
                                        step="0.1"
                                ...
                                <div className="space-y-1">
                                    <Label htmlFor="momentum_3m_min" className="text-[10px] text-muted-foreground">3 Month</Label>
                                    <Input
                                        id="momentum_3m_min"
                                        type="number"
                                        step="0.1"
                                ...
                                <div className="space-y-1">
                                    <Label htmlFor="momentum_6m_min" className="text-[10px] text-muted-foreground">6 Month</Label>
                                    <Input
                                        id="momentum_6m_min"
                                        type="number"
                                        step="0.1"
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center space-x-2 mb-4">
                                <Checkbox
                                    ...
                                    onCheckedChange={(checked) => setValue("filter_criteria.needs_analysis", checked)}
                                />
                                <Label htmlFor="needs_analysis" className="text-sm font-normal cursor-pointer">
                                    Needs Analysis (No current verdict)
                                </Label>
                            </div>
                            <Label className="text-xs text-muted-foreground">Filter by Verdict Type</Label>
                            <div className="flex flex-wrap gap-2">
                                {VERDICT_TYPES.map(type => (
                                    <Badge
                                        key={type}
                                        variant={watch("filter_criteria.verdict_types")?.includes(type) ? "default" : "outline"}
                                        className="cursor-pointer px-3 py-1"
