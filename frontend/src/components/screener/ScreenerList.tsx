                ...
                <div className="space-y-3 pb-8">
                    {screeners.length === 0 ? (
                        <div className="flex flex-col items-center justify-center p-8 text-center text-muted-foreground border border-dashed rounded-lg bg-muted/20">
                            ...
                            <Button variant="link" onClick={onCreate} className="mt-2 h-auto p-0">Create one</Button>
                        </div>
                    ) : (
                        screeners.map((screener) => (
                            <div key={screener.id} onClick={() => onSelect(screener)} className="cursor-pointer">
                                <ScreenerCard
                                    screener={screener}
