...
export function ResultRow({ result }: ResultRowProps) {
    ...
    const getVerdictBadge = (verdict: string | null) => {
        ...
        return (
            <Badge
                variant="outline"
                className={cn("font-semibold text-[10px] px-1.5 py-0 h-5", variants[verdict] || "bg-muted text-muted-foreground")}
            >
                {verdict}
            </Badge>
        );
    ...
    return (
        ...
        >
            ...
            <TableCell className="text-sm">
                {result.market_cap_formatted}
            </TableCell>
            <TableCell className="text-xs truncate max-w-[100px]" title={result.sector || ""}>
                {result.sector || "-"}
            </TableCell>
            <TableCell className={cn("text-right text-xs", getMomentumColor(result.momentum_1m))}>
                {formatPercentage(result.momentum_1m)}
            </TableCell>
            <TableCell className={cn("text-right text-xs", getMomentumColor(result.momentum_3m))}>
                {formatPercentage(result.momentum_3m)}
            </TableCell>
            <TableCell className={cn("text-right text-xs", getMomentumColor(result.momentum_6m))}>
            ...
            <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                {result.last_analyzed ? formatDistanceToNow(new Date(result.last_analyzed), { addSuffix: true }) : "-"}
            </TableCell>
            <TableCell className="max-w-[200px] truncate text-xs italic text-muted-foreground" title={result.guidance_summary || ""}>
