...
export function ResultsTable({ results, isLoading }: ResultsTableProps) {
    ...
    const handleExportCSV = () => {
        ...
        const headers = columns.map(c => c.header as string).filter(h => typeof h === 'string');
        const csvContent = [
            headers.join(','),
            ...results.map(row => [
                row.symbol,
        ...
        return (
            <div className="flex h-full min-h-[400px] flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-muted-foreground max-w-sm mt-2">
                    Try adjusting your filter criteria to see more results or run the screener again.
                </p>
            </div>
        );
    }
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <TableRow key={headerGroup.id} className="hover:bg-transparent">
                                {headerGroup.headers.map((header) => (
                                    <TableHead key={header.id} className="font-semibold text-xs whitespace-nowrap">
