...
impl JwtService {
    ...
    pub fn create_access_token(&self, user_id: Uuid, scopes: Vec<String>) -> Result<String, AppError> {
    ...
    pub fn generate_dev_keypair() -> (String, String) {
        ...
        let priv_pem = priv_key.to_pkcs1_pem(rsa::pkcs8::LineEnding::LF).expect("failed to encode private key");
        let pub_pem = pub_key.to_pkcs1_pem(rsa::pkcs8::LineEnding::LF).expect("failed to encode public key");
...
mod tests {
    ...
    fn test_jwt_flow() {
        ...
        let token = service.create_access_token(user_id, vec!["read".to_string()]).expect("Failed to create token");
        
        let claims = service.validate_access_token(&token).expect("Failed to validate token");
    ...
    fn test_refresh_token() {
        ...
        let user_id = Uuid::new_v4();
        let (token, hash) = service.create_refresh_token(user_id).expect("Failed to create refresh token");
        
        assert!(!token.is_empty());
        assert!(!hash.is_empty());
        
        // Validate the refresh token as a normal token (since it's a valid signed JWT)
        let claims = service.validate_access_token(&token).expect("Failed to validate refresh token");
        assert_eq!(claims.sub, user_id.to_string());
    }
}
