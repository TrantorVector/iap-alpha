...
async fn spawn_app() -> String {
    ...
    let mut config = Config::from_env().unwrap_or_else(|_| {
        ...
        Config {
            database_url: std::env::var("DATABASE_URL").unwrap_or_else(|_| "postgres://postgres:dev@localhost:5432/irp_dev".to_string()),
...
async fn test_refresh_with_valid_token_returns_new_access_token() {
    ...
    let resp = client.post(format!("{}/api/v1/auth/refresh", base_url))
        .json(&json!({
...
async fn test_logout_revokes_refresh_token() {
    ...
    // Logout
    let logout_resp = client.post(format!("{}/api/v1/auth/logout", base_url))
    ...
    let refresh_resp = client.post(format!("{}/api/v1/auth/refresh", base_url))
        .json(&json!({
            "refresh_token": refresh_token
        }))
        .send()
...
async fn test_protected_route_without_token_returns_401() {
    ...
    let resp = client.post(format!("{}/api/v1/auth/logout", base_url))
