...
async fn test_get_metrics_returns_quarterly_data() {
    ...
    let resp = client
        .get(format!(
            "{}/api/v1/companies/{}/metrics?period=Quarterly",
            base_url, company_id
        ))
        .header("Authorization", format!("Bearer {}", token))
        .send()
...
async fn test_get_metrics_returns_annual_data() {
    ...
    let resp = client
        .get(format!(
            "{}/api/v1/companies/{}/metrics?period=Annual",
            base_url, company_id
        ))
        .header("Authorization", format!("Bearer {}", token))
        .send()
...
async fn test_upload_document_creates_record() {
    ...
    let form = multipart::Form::new()
        ...
        .part(
            "file",
            multipart::Part::bytes(b"dummy content".to_vec())
                .file_name("test.pdf")
...
async fn test_create_and_update_verdict() {
    ...
    let resp = client
        .get(format!(
            "{}/api/v1/companies/{}/verdict/history",
            base_url, company_id
        ))
        .header("Authorization", format!("Bearer {}", token))
        .send()
